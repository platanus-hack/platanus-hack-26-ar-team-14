...
import { getCourseById, getUrgencyTone, scheduleDays } from "../lib/bitacora-data";
...
function getScheduleMap(blocks: WeeklyBlock[]) {
  return new Map(blocks.map((block) => [`${block.day}-${block.time}`, block] as const));
}
    ...
    <main className="bitacora-dashboard-shell">
      ...
      >
        <h1 className="bitacora-alert-title">
          {teacherName}, necesitamos corregir {priorityCourses.length} planes esta semana.
        </h1>
      </section>

      <section className="mt-8">
        <div className="mb-4 text-center">
          <h2 className="bitacora-calendar-title">Cursos según nivel de urgencia</h2>
              ...
              return (
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    >
                      Urgencia {course.urgency.toLowerCase()} · {course.curricularGap} OA
                  ...
                  <ul className="mt-5 grid gap-0 text-sm leading-6 text-slate-700">
                    {course.issues.slice(0, 4).map((issue, issueIndex) => (
                      ...
                      >
                        ...
                        />
                        <span className="bitacora-course-issue-text">{issue}</span>
                      </li>
                    ))}
                  </ul>
                </Link>
            ...
            {localScheduleTimes.map((time) => (
              <Fragment key={time}>
                ...
                {scheduleDays.map((day) => {
                  ...
                  return (
                    <div key={`${day}-${time}`} className="bitacora-calendar-slot">
                      <Link
                        href={`/course/${course.id}`}
