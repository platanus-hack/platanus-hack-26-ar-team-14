...
export async function loginUpstream(
  email: string,
  password: string,
): Promise<{ token: string; teacher: Teacher } | { error: string; status: number }> {
  ...
  if (!res.ok) {
    return { error: res.status === 401 ? "Credenciales inválidas." : "Error al iniciar sesión.", status: res.status };
  }
